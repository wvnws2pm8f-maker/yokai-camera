...
- 🎨 写真に妖怪を自然に合成して表示（Cloudflare Workers経由でCloudflare Workers AI(FLUX.2)を呼び出す。**未設定の場合は自動で「仮あわせ」モック合成にフォールバック**するので、Worker未設定でも動作確認はできる）
...
- コレクションには `referenceImage`（デザイン確認用に生成したイラストのプレビュー画像）
  というフィールドもあり、実際にAI合成される前に見た目のイメージを確認・調整できる
